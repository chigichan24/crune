...
- **H1 (retrieval quality): 条件付き合格（クレンジング実装後）。** クリーンなプロンプトでは明確に blob を上回る。生テキストにツール出力ノイズが多く前処理が品質を律速する、というのが本 PoC 最大の学びで、**この最優先項目は本 PR で `extractChunks` のクレンジング（後述）として実装済み**。off-domain クエリでの最終的な品質評価は #33 の合成 A/B（#20 evaluator）に委ねる。

### クレンジング後の再測定（本 PR で対応済み）

PoC の最優先課題だった「チャンクテキストのクレンジング」を `cleanseNoise()` として実装し、実コーパス（40セッション / 249チャンク）で再測定した:

- ノイズ除去（`<bash-stdout>`/`<bash-stderr>`/`<task-notification>`/`<system-reminder>`/`npm notice`/`Full transcript available at:`/`API Error:` 等の除去 + stray-tag 除去 + assistant 長 1000 字キャップで intent 比重を上げる）後、検索上位スニペットから漏洩していたツール出力・ファイルパス断片が消え、実コンテンツが上位に来るようになった（例: "fix the authentication token bug" → 漏洩パスではなく実発話 score 0.854）。
- footprint: index.bin 93.4 KB（249×384 int8, 384 B/chunk）— 引き続き合格。
- latency: p50 ~10 ms（p95 はサンプル n=20 の単発外れで変動）— 引き続き合格。
- **残る限界（誠実な記録）:** 本コーパスは crune 自身の開発ログ（日本語中心）で、PoC の英語サンプルクエリ（auth/db）と**ドメインが一致しない**ため、A/B での「明確な勝ち」は本コーパスでは判定しづらい。多言語モデルのクロスリンガル検索自体は機能している。retrieval 品質の真の評価軸は、#33 で実際の合成出力を #20 evaluator で before/after 比較すること。
...
## 推奨: **Go（条件付き）** — #33 / #34 を進め、品質ゲートは #33 の evaluator A/B に置く

当初判断は Hold（最優先のクレンジング未対応のため）だった。**その最優先項目を本 PR で実装・再測定した結果、Hold の根拠が解消した**ので、判断を **Go（条件付き）** に更新する。

- 配管（埋め込み→int8インデックス→ハイブリッド検索→多様化）は完成。footprint・latency は合格。クレンジング後は検索上位から漏洩ノイズが消えた。
- ただし `--embed` は **デフォルト OFF（オプトイン）を維持** する。retrieval 品質の最終判定は #33 で実際の合成出力を #20 evaluator で before/after 比較してから（本コーパスの off-domain クエリだけでは断定しないため）。

Go の条件（#33 / #34 と並行 or 直後に潰す、いずれも独立・非ブロッキング）:
1. ✅ **チャンクテキストのクレンジング（最優先・完了）。** `cleanseNoise()` をツール出力/システム通知/npm chatter/stray-tag 除去 + assistant 長キャップとして実装済み。
2. **スループット対策。** 実測 ~8 chunks/sec（CPU/WASM, MiniLM-L12）。数千チャンクで数分〜十数分。quantized ONNX モデル / 増分埋め込み（変更セッションのみ再計算）で改善。`--embed` をオプトインに留めた設計で当面は許容。
3. **meta.json の配信最適化。** snippet 込みで index.bin より大きい。snippet 短縮 or 別ファイル化で初期ロードを軽くする。

撤回（Pivot）としない根拠: ノイズ除去は局所的な前処理変更で対応でき（実装済み）、retriever 本体・埋め込みモデルの選定は妥当だった。配管はそのまま #33（検索強化合成）/ #34（検索UI）の土台として再利用する。
